...
dataset = load_dataset(os.path.join(config.data.input_path, 'chatgpt_data/ConflictQA'),'ConflictQA-popQA-gpt4')
...
for filename in results_files:
    if '.jsonlines' in filename:
        os.remove(os.path.join(config.data.output_path, filename))
        print('Delete file', os.path.join(config.data.output_path, filename))

facts = []
for label_idx in range(config.data.num_of_labels):
    fact = pd.read_csv(os.path.join(config.data.input_path, 'fact_%s.txt' % label_idx), sep = '------', header = None, engine = 'python')
...
for i in range(len(facts[0])):
    ...
    for position_index in range(question_length):
        with jsonlines.open(os.path.join(config.data.output_path, 'hidden_states_-%s.jsonlines' % (position_index+1)), 'a') as f:
...
with open(os.path.join(config.data.output_path, 'acc.txt'), 'a') as f:
