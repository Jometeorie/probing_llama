...
parser.add_argument('--fact_idx', type=int, default=0) # -1表示全部
...
fact = pd.read_csv(os.path.join(args.root_path, config.data.input_path, 'commonsense_evidence/fact_0.txt'), sep = '------', header = None, engine = 'python')
if args.fact_idx == -1:
...
for fact_idx in fact_idx_list:
    ...
    for i in range(len(facts[0])):
        ...
        ground_truth = dataset['test']['ground_truth'][fact_idx]
    ...
    if args.record_acc:
        with open(os.path.join(args.root_path, config.data.output_path, 'acc_commonsense.txt'), 'a') as f:
