...
for position_index in range(hidden_step):
    ...
    for layer_idx in [0, 10, 20]:
        for position, acc_list in acc_of_each_position.items():
            X = hidden_states_of_each_layer[position][layer_idx]
            y = np.array(labels_of_each_layer[position][layer_idx])
            pca = PCA(n_components=2)
            pca = pca.fit(X)
            X_dr = pca.transform(X)
            plt.scatter(X_dr[y==0,0], X_dr[y==0,1], c='red')
            plt.scatter(X_dr[y==1,0], X_dr[y==1,1], c='green')
            plt.savefig(os.path.join('temp/%s_%s.jpg' % (position, layer_idx)))
            plt.close()
            np.save("temp/%s_%s_X_dr.npy"  % (position, layer_idx), X_dr)
            np.save("temp/%s_%s_y.npy"  % (position, layer_idx), y)
            ...
            print('Hyb: %s, Hyx: %s, pvi: %s, acc: %s' % (H_yb, H_yx, Vi, correct.item() / len(y_test)))
